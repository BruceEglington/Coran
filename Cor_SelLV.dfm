object fmSelectNumVectors: TfmSelectNumVectors
  Left = 396
  Top = 295
  Width = 463
  Height = 170
  Caption = 'Select number of eigenvectors for approximation'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 100
    Top = 16
    Width = 261
    Height = 13
    Alignment = taCenter
    Caption = 'How many eigenvectors do you wish to keep?'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lMax: TLabel
    Left = 180
    Top = 32
    Width = 75
    Height = 13
    Alignment = taCenter
    Caption = '(Maximum is 10)'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object eNumEigenVectorsSelected: TEdit
    Left = 200
    Top = 56
    Width = 29
    Height = 21
    TabOrder = 0
    Text = '2'
  end
  object bbOK: TBitBtn
    Left = 180
    Top = 100
    Width = 75
    Height = 25
    TabOrder = 1
    OnClick = bbOKClick
    Kind = bkOK
  end
end
