...
type
  TfmSheetImportDiscrimFac = class(TForm)
    pControl: TPanel;
    sbSheet: TStatusBar;
    bbOpenSheet: TBitBtn;
...
var
  fmSheetImportDiscrimFac: TfmSheetImportDiscrimFac;
...
function TfmSheetImportDiscrimFac.GetStringFromCell(iRow,iCol : integer) : string;
begin
  Result := Xls.GetStringFromCell(iRow,iCol);
end;

procedure TfmSheetImportDiscrimFac.FillTabs;
var
  s: integer;
begin
  Tabs.Tabs.Clear;
...
procedure TfmSheetImportDiscrimFac.ClearGrid;
var
  r: integer;
begin
...
procedure TfmSheetImportDiscrimFac.FillGrid(const Formatted: boolean);
var
  r, c, cIndex: Integer;
  v: TCellValue;
begin
...
function TfmSheetImportDiscrimFac.ConvertCol2Int(AnyString : string) : integer;
var
  itmp    : integer;
  tmpStr  : string;
  tmpChar : char;
...
procedure TfmSheetImportDiscrimFac.bbOpenSheetClick(Sender: TObject);
var
  tmpStr    : string;
  i : integer;
begin
...
procedure TfmSheetImportDiscrimFac.bbImportClick(Sender: TObject);
var
  j      : integer;
  iii    : integer;
  iCode  : integer;
...
begin
  ...
  begin
    sbSheet.SimpleText := 'Clearing existing values';
    repeat
      dmCor.EigenVec.Delete;
    until dmCor.EigenVec.Bof;
  end;
  sbSheet.SimpleText := 'Appending new values';
  ii := 0;
  ...
  begin
    ...
    begin
      jj := jj + 1;
      try
        //tmpStr := FlexCelImport1.CellValue[iii,j];
        tmpStr := Xls.GetStringFromCell(iii,j);
        dmCor.EigenVec.Fields[1].AsString := tmpStr;
      except
      end;
    end;
    dmCor.EigenVec.Post;
...
procedure TfmSheetImportDiscrimFac.FormShow(Sender: TObject);
var
  i, j : integer;
begin
...
procedure TfmSheetImportDiscrimFac.bbCancelClick(Sender: TObject);
begin
  ModalResult := mrNone;
  Close;
end;
...
procedure TfmSheetImportDiscrimFac.sbFindLastRowClick(Sender: TObject);
var
  iCode : integer;
  tmpStr : string;
  i,j : integer;
...
procedure TfmSheetImportDiscrimFac.cbSheetNameChange(Sender: TObject);
begin
  //ImportSheetNumber := cbSheetName.ItemIndex+1;
